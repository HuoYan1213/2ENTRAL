...
class RecycleBinController {
    ...
    public function getItems($page = 1, $limit = 10, $filters = []) {
        ...
        $productQuery = "SELECT ProductID as ID, ProductName as Name, 'Product' as Type, ImagePath, Category as Detail FROM products WHERE IsActive = 'Inactive'";
        $supplierQuery = "SELECT SupplierID as ID, SupplierName as Name, 'Supplier' as Type, ImagePath, Email as Detail FROM suppliers WHERE IsActive = 'Inactive'";
        ...
        // Note: The inner SQL already has WHERE clauses, so we need to be careful with binding.
        // A simpler way for count is to execute the full query without limit/offset first? 
        // Or just reconstruct the count query.
        
        // Let's use a cleaner approach for Count:
    ...
    public function bulkAction() {
        ...
        foreach ($items as $item) {
            ...
            // Simulate internal POST request logic or call private methods
            // For simplicity, we'll just replicate logic briefly or call helper
            // But since we are in the same class, let's just do logic directly.
            
            $table = ($type === 'Product') ? 'products' : 'suppliers';
            $idCol = ($type === 'Product') ? 'ProductID' : 'SupplierID';

            if ($actionType === 'restore') {
                // Simplified restore for bulk (skipping complex email checks for brevity, or assume user handles errors)
                $sql = "UPDATE $table SET IsActive = 'Active' WHERE $idCol = ?";
                $stmt = $this->conn->prepare($sql);
